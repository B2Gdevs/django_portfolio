...
django_data_path = "predictions/pickled/data/"
django_model_path = "predictions/pickled/models/"
...
def create_row(team, opposing_team, quarter,
    ...
    if os.path.exists(data_path):
        ...
        data_path = os.path.join(django_data_path, "nba_X_training_data.pkl")
        if os.path.exists(data_path):
            dataframe = load_binary(data_path)
        else:
...
def predict(team, opposing_team, quarter):
    ...
    """
    model = load_binary(os.path.join(django_model_path,
                                     'nba_quarter_under_20_SVM.pkl'))
    row = create_row(team, opposing_team, quarter)
